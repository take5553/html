...
ちなみに自分のドキュメントルートの場所は`D:\work\HTML\raspberrypi-server\test\html`としている。
...
VSCodeには「ワークスペース」という概念がある。これは実体としては`.vscode-workspace`という拡張子を持つファイルで、その中に
...
今回の掲示板プロジェクトではそこまで恩恵を受けるわけではないけど、プロジェクトの規模が大きくなってくると複数フォルダを1度に開くことができたり、VSCodeの設定をプロジェクト向けにカスタマイズできたりして便利になるらしい。作って概念に慣れておこう。

フォルダを追加する場合は「フォルダーをワークスペースに追加...」をクリック。保存する場合は「名前を付けてワークスペースを保存...」をクリック。

![image-20201206153237800](image/preparation/image-20201206153237800.png)

どこでもいいけど、`bbs`フォルダに保存しておく。

[続く。](preparation2.html)